...
class Earnings {
  ...
  async _response(earning) {
    // console.log('In Earnings._response with earning: ', earning);
    let result = await this.constructor.Earning(earning);
    return result;
  }
  ...
  async getEarnings(filter, limitOptions) {
    ...
    let formattedEarnings = await Promise.all(
      earnings.map((row) => {
        // console.log('In Earnings.getEarnings with row: ', row);
